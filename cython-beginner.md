...
この記事を書こうとした背景としては, Cythonの解説記事は公式も含めて少なからずあるものの, それらが簡単な関数を高速化するだけで終わってることが多く, 1つのまとまったプログラムとして参考にできるものが見つからなかったからです。また, Cythonの環境構築から基本, および高速化のコツなどを一気通貫して学ぶことのできる記事もありませんでした。
そこでこの記事では, Cythonの環境構築から始め, Cythonの基本的な書き方を簡単に紹介した後, マルチエージェントシミュレーションを実践例として高速化のコツや注意点を説明します。なお, シミュレーションのコードはあくまでCython実装の例として用意しているため, シミュレーションの設計自体は詳細に説明しません。

私が研究でCythonを実装する際や, 本記事の執筆にあたり, **参考資料**セクションに載せている記事に多いに助けられました。ぜひそれらを参考にした後, 必要であればこの記事を読んでください。
個人的に, 新しいツールを学ぶ際には, とにかくたくさんの動かせるサンプルコードに触れることが重要だと思っているので, この記事が参考になれば幸いです。

本記事で用いたコードは以下のリンク先から入手できます。
https://github.com/Tomoki-Osaki/cython-beginner-simulation
...
Windows11およびM2 macOS (ver. Tahoe 26.4)でも動作確認をしています。
...
(cython) conda install seaborn
```
なお, seabornは可視化用のライブラリですが, numpyやmatplotlib, pandasなどのデータ分析でよく使うライブラリを同時にインストールできるので, データ分析をする場合はおすすめです。そうでない場合は, numpyだけインストールしてください。
...
```
### コンパイラのインストール
また, Cythonを使う際にはCコンパイラが必要なので, それぞれのOSに合わせてインストールします。
公式サイトのOSごとのコンパイラのインストール方法は[こちら](https://cython.readthedocs.io/en/latest/src/quickstart/install.html)です。

Linuxでは, sudo apt install build-essentialでインストールできます。
macOSでは, Apple Storeや[Xcode - Apple Developer](https://developer.apple.com/)からXcodeをインストールします。

Windowsでは, [Microsoft C++ Build Tools - Visual Studio](https://visualstudio.microsoft.com/visual-cpp-build-tools/)からBuild Toolsをインストールします。その後, Visual Studio Build Toolsを立ち上げ, ワークロードの選択画面で「C++によるデスクトップ開発」にチェックを入れてインストールします。
なお, Windowsでコンパイラをインストールしていない状態でsetup.pyを実行しようとすると, 以下のメッセージが出ます。上述のBuild Toolsへのリンクは, 下のメッセージに載っているものです。
```
...
3. .pyファイルで, 2の出力ファイル(.so / .pyd)をインポートして使用する

この他に.pydファイルや.hファイルなどを利用してC/C++で書いた関数を使用するなどの方法もありますが, この記事では扱いません。

なお, .pyxファイルを変更した場合, 変更前の.pyxファイルからインポートしたモジュールを使用しているプロセスをキルしてからsetup.pyを再度実行しないと, 出力ファイル (.so / .pyd) に.pyxファイルへの変更が反映されません。
...
これ以降は, このシミュレーションコードのうち, Cython高速化に関わるヒントや, つまずきやすいポイントを解説しながら進めていきます。説明しないコードもあります。シミュレーションの仕様の詳細な説明はしませんが, 概要を記述しておきます。
...
本記事のシミュレーションプログラムは, 「Pythonによる数値計算とシミュレーション」の「6.2.2 マルチエージェントシミュレーションプログラム」を参考に改変して実装しています。

https://www.ohmsha.co.jp/book/9784274221705/

なお, 上記リンク先のページに本のソースコードのリンクがありますが, 「本ファイルは、本書をお買い求めになった方のみご利用いただけます。」とありますので, 利用にはご注意ください。この記事でも, ソースコードをそのままではなく, かなりリファクタリングして使用しています。

### シミュレーション全コード(111行)
まずはCythonコードであるinfection_cy.pyxファイルです。なお, ここに載せているコードではコメントを省いていますが, GitHubにはコメント付きのファイルもアップロードしていますので, 適宜参照してください。
なお, 記事中のコードブロックのうち, 「〇〇の例」というコメントが冒頭にあるものは, シミュレーションのコード由来ではありません。
```python
...
```
次のコードはライブラリのインポートです。
Cython (.pyx) に特異な点は, cimportでC/C++の関数や変数をインポートできることです。3行目のcimport numpy as cnpでは, NumPyのC言語APIを利用できるようにしています。これにより, NumPyの配列をデータ型として宣言することができるようになります (e.g., cnp.ndarray[cnp.int_t, ndim=2])。公式サイトの説明は[こちら](https://cython.readthedocs.io/en/latest/src/tutorial/numpy.html#adding-types)。

注意点として, .pyxファイル内での通常のnumpyインポート(import numpy as np)とエイリアスが被らないようにしてください。以下の例では, import numpy as np由来のモジュールとcimport numpy as np由来のモジュールが混在してしまっていて, 非常にバグが起きやすい状況となってしまっています。
```python
# ダメな例
import numpy as np
cimport numpy as np
...
```
このマジックコマンドにより, コンパイラがC言語用のものからC++用のものになり, C++のモジュールをインポートできるようになります。このマジックコマンドなしでlibcppからインポートしようとするとコンパイルエラーが発生します。
なお, 利用できるC/C++の関数や変数は, [公式GitHub](https://github.com/cython/cython/tree/master/Cython/Includes)のlibcおよびlibcppディレクトリ内のファイルから確認できます。
...
### NumPy配列と型付きメモリービュー
クラスの定義中にあった, double[:,:,:] pos や, int[:] if_infected はCythonでのNumPyとの連携で非常に重要な部分です。
...
### インデックスアクセスによる配列の準備
__init__中で, Cythonに特徴的なself.pos以下です。
まずself.pos = np.empty([num_agents, 2])のように, 空の配列を用意します。その後, forループで配列の各要素にインデックスアクセスをして, c_rand関数で生成した乱数を代入しています。このように, Cytohnではまず配列を準備してforループでアクセスしていくという, C言語に似た書き方をします。
...
### 型付きメモリービューの計算
calc_next_stateメソッドのブロックは飛ばして, check_if_infectedメソッドです。クラスのメソッドも, 上で見た関数のように.pyxファイル内だけで呼び出せるcdefか, .pyファイルでも呼び出せるcpdefで定義します。また, 戻り値が無い場合は, voidとして設定します。
...
└── temp.linux-x86_64-cpython-312
    └── infection_cy.o
```
このうち使用するのは.soファイル (Windowsでは.pyd) で, .pyファイル内のimport文で.pyxファイルの名前を指定してインポートします。ただし, 上記のような階層構造だとインポートを行う時にパスの設定が面倒なので, -iオプション (または--inplace) をつけます。これにより, setup.pyファイルのあるディレクトリに.soファイルがコピーされます。
```
...
## まとめ
Cythonは, Pythonと比べてやはり柔軟性は劣りますが, その分圧倒的な速さと, 静的型付けによるコードの堅牢性が魅力だと思います。公式の開発はつい最近も更新があるぐらい活発なのですが, 残念なことに日本語で書かれている記事は少なく, それが入門ハードルを上げているように感じます。私はCythonに流行って欲しいので, この記事でCythonを始めてみようと思う人がいれば幸いです。
